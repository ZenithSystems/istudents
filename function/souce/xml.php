 ...
 * Xml
 *
 * function ที่เกี่ยวกับ array, xml, json
 *
 * @package Name
 * @subpackage function
 * @category souce
...
class xml {
  ...
  public function convertXmlToArray($xmlContent) {
    ...
    if ($ret === false) {
      $this->errorLog[] = "Invalid XML content: $xmlContent in function: " . __FUNCTION__ . ' on line: ' . __LINE__ . ' in filename= ' . 'C:\AppServ\www\desingbyatom\Minecraft\imc\include/class.xml1.php';
      return false;
    } else {
      $return = $this->parseXml($ret);
      if ($return === false) {
        $this->errorLog[] = 'Failed to parse XML content in function: ' . __FUNCTION__ . ' on line: ' . __LINE__ . ' in filename= ' . 'C:\AppServ\www\desingbyatom\Minecraft\imc\include/class.xml1.php';
        return false;
      }
    }
    return $return;
  ...
  private function parseArray($array) {
    ...
    if (is_array($array)) {
      foreach ($array as $k => $v) {
        if (trim($k) == '') {
          $this->errorLog[] = 'Array needs to be associative as parameter in function: ' . __FUNCTION__ . ' on line: ' . __LINE__ . ' in filename= ' . 'C:\AppServ\www\desingbyatom\Minecraft\imc\include/class.xml1.php';
          return false;
        } else {
          if (is_numeric($k)) {
            $k = "nodeValue$k";
      ...
      $this->errorLog[] = 'Invalid array in function: ' . __FUNCTION__ . ' on line: ' . __LINE__ . ' in filename= ' . 'C:\AppServ\www\desingbyatom\Minecraft\imc\include/class.xml1.php';
      return false;
    }
    return $return;
  }
  ...
  public function convertJsonToXML($json) {
    if (!is_string($json)) {
      $this->errorLog[] = 'The first parameter should to be string in function: ' . __FUNCTION__ . ' on line: ' . __LINE__ . ' in filename= ' . 'C:\AppServ\www\desingbyatom\Minecraft\imc\include/class.xml1.php';
      return false;
    }
    $array = json_decode($json, true);
    if ($array === false) {
      $this->errorLog[] = 'Failed to decode JSON in function: ' . __FUNCTION__ . ' on line: ' . __LINE__ . ' in filename= ' . 'C:\AppServ\www\desingbyatom\Minecraft\imc\include/class.xml1.php';
      return false;
    } else {
